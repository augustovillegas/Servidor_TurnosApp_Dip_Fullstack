...
- Identidad: `nombre`, `apellido`, `email` (único, lowercase), `passwordHash`.
- Academico: `modulo` y `moduloSlug` (mismo enum `MODULE_NAME_VALUES`), `cohorte`, `isRecursante`.
- Accesos: `role` (`alumno/profesor/superadmin`), `isApproved`, `status` (`Pendiente/Aprobado/Rechazado`).
...
### slotService.mjs (operaciones protegidas para la app principal)
- Normaliza estados (`REVIEW_STATUS_CANONICAL`).
- `crear`: exige `profesor/superadmin`, valida `assignment` como ObjectId y setea `reviewStatus="A revisar"`.
- `solicitarTurno`: alumno debe pertenecer al mismo `cohort`, evita doble reserva.
- `cancelarTurno`: solo si el turno le pertenece al alumno, resetea `reviewStatus`.
- `actualizarEstadoRevision`: solo roles altos, normaliza texto recibido (`aprobado`, `pendiente`, `cancelado`).
- `obtenerPorUsuario`, `obtenerTurnosPorFiltro`, `obtenerSolicitudesPorAlumno`: exponen filtros básicos.

### submissionService.mjs (relaciona entregas con slots)
- Usa `ReviewSlot` y `Assignment` directos además del repositorio.
- Normaliza estados y evita modificaciones en `FINAL_STATES` por alumnos.
- `crearEntrega(slotId, user, body)`: obliga a que el alumno sea dueño del turno y a proporcionar `githubLink`, clona `assignment` desde el slot y sincroniza `reviewStatus/estado`.
- `obtenerEntregasPorUsuario`: bloquea que un alumno lea entregas ajenas.
- `obtenerEntregaPorId`: profesores solo si son dueños del assignment.
- `actualizarEntrega`: alumnos solo si no está finalizada; profesores/superadmin pueden fijar estado.
- `eliminarEntrega`: mismo gating que `actualizar`.

### frontendSlotService.mjs (servicios pensados para panel admin/frontend)
- Mapea objetos crudos a un DTO (`toFrontend`) con `modulo`, `profesorId`, `solicitanteNombre`, `estado`, `duracion`, etc.
- Incluye parsers de fechas (ISO o `DD/MM/YYYY`), resolución de `module` (`moduleMap`), sincronización entre `estado` y `reviewStatus` (diccionarios `ESTADO_TO_REVIEW_STATUS` y viceversa).
- `listarTurnos(query)`: filtra por `cohort`, `reviewNumber`, `userId`, `modulo`, `estado` (acepta strings o ObjectId válido).
- `crearTurno/actualizarTurno/eliminarTurno`: limpian payload, castea objectIds, recalculan `estado` y `reviewStatus` antes de persistir.

### frontendSubmissionService.mjs
- Expone DTO compacto (`id`, `sprint`, `githubLink`, `alumno`, `estado`, `modulo`, `fechaEntrega`).
- `listarEntregas(query)`: filtra por `estado`, `modulo` (usa `moduleMap`), `sprint`, `userId`; hace `populate` de `student` y `assignment` para completar nombres.
- `crearEntrega/actualizarEntrega`: validan `githubLink`, aceptan `alumnoId`, `assignmentId`, sincronizan `estado/reviewStatus`.

### frontendUserService.mjs
- Reutiliza `authService.register` para crear usuarios desde el panel y luego fuerza `status`/`isApproved` según el dato cargado.
- Exporta `listarUsuarios`, `obtenerUsuario`, `crearUsuario`, `actualizarUsuario`, `eliminarUsuario`. Normaliza `rol`, `estado`, deduce `cohort` desde `modulo` cuando es posible.

### userService.mjs
- Capa fina que delega CRUD a `userRepository`; se usa desde el middleware `auth` para cargar el documento completo.
...
### `/turnos` (panel administrativo, reuse `slotController` + `frontendSlotService`)
Middlewares globales: `auth`, `allowRoles(profesor,superadmin)` (aplicados con `router.use`).

| Método | Ruta | Controlador -> Servicio | Notas |
| --- | --- | --- | --- |
| GET | `/turnos/` | `listarTurnosFrontendController` -> `frontendSlotService.listarTurnos` | Devuelve DTO listo para UI filtrable por query.
| GET | `/turnos/:id` | `obtenerTurnoFrontendController` -> `frontendSlotService.obtenerTurno` | Valida ObjectId.
| POST | `/turnos/` | `crearTurnoFrontendController` -> `frontendSlotService.crearTurno` | Crea y retorna DTO.
| PUT | `/turnos/:id` | `actualizarTurnoFrontendController` -> `frontendSlotService.actualizarTurno` | Recalcula estado/estudiante.
| DELETE | `/turnos/:id` | `eliminarTurnoFrontendController` -> `frontendSlotService.eliminarTurno` | 204.
...
| --- | --- | --- |
| GET | `/entregas/` | `listarEntregasFrontendController` -> `frontendSubmissionService.listarEntregas` |
| GET | `/entregas/:id` | `obtenerEntregaFrontendController` -> `frontendSubmissionService.obtenerEntrega` |
| POST | `/entregas/` | `crearEntregaFrontendController` -> `frontendSubmissionService.crearEntrega` |
| PUT | `/entregas/:id` | `actualizarEntregaFrontendController` -> `frontendSubmissionService.actualizarEntrega` |
| DELETE | `/entregas/:id` | `eliminarEntregaFrontendController` -> `frontendSubmissionService.eliminarEntrega` |
...
| Método | Ruta | Controlador -> Servicio |
| --- | --- | --- |
| GET | `/usuarios/` | `listarUsuariosFrontendController` -> `frontendUserService.listarUsuarios` |
| GET | `/usuarios/:id` | `obtenerUsuarioFrontendController` -> `frontendUserService.obtenerUsuario` |
| POST | `/usuarios/` | `crearUsuarioFrontendController` -> `frontendUserService.crearUsuario` |
| PUT | `/usuarios/:id` | `actualizarUsuarioFrontendController` -> `frontendUserService.actualizarUsuario` |
| DELETE | `/usuarios/:id` | `eliminarUsuarioFrontendController` -> `frontendUserService.eliminarUsuario` |
...
5. **Turnos Frontend DTO:** campos disponibles (via `frontendSlotService`): `id`, `modulo`, `profesorId`, `zoomLink`, `fecha` (`date`, `start`, `end`, `startTime`, `endTime`), `estado`, `reviewStatus`, `solicitanteId`, `solicitanteNombre`, `comentarios`, etc. Mantener sincronizadas las opciones con `VALID_ESTADOS`.
